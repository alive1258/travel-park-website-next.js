...
export const testimonialApi = baseApi.injectEndpoints({
  endpoints: (builder) => ({
    ...
    // 3. GET ACTIVE TESTIMONIALS (public homepage)
    getActiveTestimonials: builder.query<ActiveTestimonialsResponse, void>({
      query: () => ({
        url: `${TESTIMONIAL_URL}/active`,
        method: "GET",
      }),
      providesTags: [tagTypes.testimonials],
    }),

    // 4. GET SINGLE TESTIMONIAL BY ID
