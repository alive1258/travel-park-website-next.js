...
const ALL_SERVICES_PATH = "/dashboard/services/all-services";

const EditService: React.FC<EditServiceProps> = ({ id }) => {
  ...
  // from `serviceData` stays put instead of being cleared.
  useEffect(() => {
  ...
  const onSubmit: SubmitHandler<EditServiceFormValues> = async (values) => {
    try {
    ...
    <div className="rounded-lg border bg-white border-gray-200 overflow-hidden p-6">
      <PageHeader
        ...
        breadcrumbs={[
          ...
          { title: "Our Services", link: ALL_SERVICES_PATH },
          { title: "Edit Service" },
        ]}
      />
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="mt-8 space-y-6">
        ...
        <div className="flex justify-end gap-4 mt-6">
          ...
          <GradientButton
            type="submit"
            text={isUpdating ? "Updating..." : "Update Service"}
            icon={Save}
            disabled={isUpdating}
          />
        </div>
