...
interface EditExperienceFormValues {
  title: string;
  description?: string;
  position?: number;
  is_active: boolean;
...
const EditExperience = ({ id }: EditExperienceProps) => {
  const router = useRouter();

  const [imagePreview, setImagePreview] = useState<string | null>(null);

  const { data: experienceData, isLoading: isFetching } =
    useGetSingleExperienceQuery(id);
  ...
  const {
    ...
    reset,
    formState: { errors },
  } = useForm<EditExperienceFormValues>();
  ...
  // from `experienceData` stays put instead of being cleared.
  useEffect(() => {
  ...
  useEffect(() => {
    if (experienceData?.data) {
      const item = experienceData.data;
      reset({
        title: item.title || "",
  ...
  const onSubmit: SubmitHandler<EditExperienceFormValues> = async (
    values,
  ) => {
    try {
      ...
      await updateExperience({ id, data: formData }).unwrap();
    ...
    <div className="rounded-lg border bg-white border-gray-200 overflow-hidden p-6">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="mt-8 space-y-6">
        ...
        <div className="flex justify-end gap-4 mt-6">
          ...
          <GradientButton
            type="submit"
            text={isUpdating ? "Updating..." : "Update Experience"}
            icon={Save}
            disabled={isUpdating}
          />
        </div>
