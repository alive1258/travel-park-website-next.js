...
interface EditAboutExploreProps {
  id: string;
}

interface EditAboutExploreFormValues {
  title: string;
  description: string;
  href: string;
...
const EditAboutExplore: React.FC<EditAboutExploreProps> = ({ id }) => {
  const router = useRouter();

  const { data: cardData, isLoading: isFetching } =
    useGetSingleAboutExploreQuery(id);
  const [updateAboutExplore, { isLoading: isUpdating }] =
    useUpdateAboutExploreMutation();
  ...
  const {
    ...
    reset,
    formState: { errors },
  } = useForm<EditAboutExploreFormValues>();
  ...
  const onSubmit: SubmitHandler<EditAboutExploreFormValues> = async (
    values,
  ) => {
    try {
      await updateAboutExplore({
        id,
        data: {
          title: values.title,
...
export default EditAboutExplore;
