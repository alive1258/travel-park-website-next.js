...
import { SustainabilityPillar } from "@/src/types/sustainabilityPillarType";
...
const AllSustainabilityPillars: React.FC = () => {
  ...
  const [deleteSustainabilityPillar] = useDeleteSustainabilityPillarMutation();

  const pillars: SustainabilityPillar[] = data?.data || [];
  ...
  const handleDeletePillar = async (pillar: SustainabilityPillar) => {
    try {
      const result = await Swal.fire({
        title: "Are you sure?",
        text: `Delete pillar "${pillar.title}"?`,
        icon: "warning",
      ...
      await deleteSustainabilityPillar(pillar.id).unwrap();

      await Swal.fire({
        icon: "success",
        title: "Deleted!",
        text: `Pillar "${pillar.title}" has been deleted.`,
        timer: 1000,
      ...
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 p-6 border-b border-gray-200">
        <div>
          ...
          <p className="text-sm text-gray-500">
            Manage the &quot;Four Pillars&quot; cards on the Sustainability page
          </p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3 w-full lg:w-auto">
          ...
          <Link href="/dashboard/sustainability-pillars/add-sustainability-pillar">
          ...
          <tbody>
            {pillars?.length > 0 ? (
              pillars.map((pillar, index) => {
                const Icon = SUSTAINABILITY_PILLAR_ICON_MAP[pillar.icon];
                return (
                  <tr
                    key={pillar.id}
                  ...
                  >
                    ...
                    <td className="px-5 py-3">
                      <div className="flex justify-center gap-2">
                        <Link
                          href={`/dashboard/sustainability-pillars/edit-sustainability-pillar/${pillar.id}`}
                        >
                          <button
              ...
              <tr>
                <td colSpan={7} className="py-10 text-center text-gray-500">
                  No Sustainability Pillars found.
                </td>
              </tr>
            )}
          </tbody>
...
export default AllSustainabilityPillars;
