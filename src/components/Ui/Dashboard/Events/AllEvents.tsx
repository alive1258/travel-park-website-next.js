...
import { EventItem } from "@/src/types/eventType";
  ...
  const handleDeleteEvent = async (event: EventItem) => {
    try {
      ...
      await Swal.fire({
        icon: "success",
        title: "Deleted!",
        text: `Event "${event.name}" has been deleted.`,
        timer: 1000,
      ...
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 p-6 border-b border-gray-200">
        <div>
          ...
          <p className="text-sm text-gray-500">
            Manage the events list on the Events & Boat Shows page
          </p>
        </div>
                ...
                >
                  ...
                  <td className="px-5 py-3">
                    <div className="flex justify-center gap-2">
                      <Link href={`/dashboard/events/edit-event/${event.id}`}>
                        <button
