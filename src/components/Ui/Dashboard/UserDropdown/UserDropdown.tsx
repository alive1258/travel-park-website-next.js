...
import { ApiError, LogoutResponse, User } from "@/src/types/authType";
...
export const UserDropdown: React.FC = () => {
  ...
  const roleContext = myInfo?.data?.role;
  const permissions = myInfo?.data?.permissions;
  ...
  useEffect(() => {
    ...
    if (roleContext && permissions) {
      dispatch(storeRoleContext({ role: roleContext, permissions }));
    }
  }, [currentUser, userData, roleContext, permissions, dispatch]);
