...
import { useCreateSustainabilityRoadmapMutation } from "@/src/redux/api/sustainabilityRoadmapApi";

interface AddSustainabilityRoadmapFormValues {
  year: string;
  milestone: string;
  position?: number;
  is_active: boolean;
...
const AddSustainabilityRoadmap = () => {
  const router = useRouter();

  const [createSustainabilityRoadmap, { isLoading }] =
  ...
  const {
    ...
    reset,
    formState: { errors },
  } = useForm<AddSustainabilityRoadmapFormValues>({
    defaultValues: {
      year: "",
      milestone: "",
      position: 1,
  ...
  const onSubmit: SubmitHandler<AddSustainabilityRoadmapFormValues> = async (
    values,
  ) => {
    try {
      await createSustainabilityRoadmap({
        year: values.year,
    ...
    <div className="rounded-lg border bg-white border-gray-200 overflow-hidden p-6">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="mt-8 space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="col-span-full flex flex-col gap-1">
            ...
            <textarea
              id="milestone"
              rows={3}
              placeholder="Completed hybrid-electric refits across the first eight yachts in the fleet."
              {...register("milestone", {
        ...
        <div className="flex justify-end gap-4 mt-6">
          ...
          <GradientButton
            type="submit"
            text={isLoading ? "Saving..." : "Create Item"}
            icon={Plus}
            disabled={isLoading}
          />
        </div>
...
export default AddSustainabilityRoadmap;
