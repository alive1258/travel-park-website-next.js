...
import { AboutStat } from "@/src/types/aboutStatType";
...
const AllAboutStats: React.FC = () => {
  const [searchValue, setSearchValue] = useState("");
  const [currentPage, setCurrentPage] = useState(1);

  const debouncedSearch = useDebounce(searchValue, 500);

  const { data, isLoading, isFetching, refetch } = useGetAllAboutStatsQuery({
    search: (debouncedSearch as string) || undefined,
    page: currentPage,
    limit: LIMIT,
  });

  const [deleteAboutStat] = useDeleteAboutStatMutation();

  const stats: AboutStat[] = data?.data || [];
  ...
  const handleDeleteStat = async (stat: AboutStat) => {
    try {
      const result = await Swal.fire({
        title: "Are you sure?",
        text: `Delete stat "${stat.label}"?`,
        icon: "warning",
      ...
      await deleteAboutStat(stat.id).unwrap();

      await Swal.fire({
        icon: "success",
        title: "Deleted!",
        text: `Stat "${stat.label}" has been deleted.`,
        timer: 1000,
      ...
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 p-6 border-b border-gray-200">
        <div>
          ...
          <p className="text-sm text-gray-500">
            Manage the stat strip on the About page
          </p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3 w-full lg:w-auto">
          ...
          <Link href="/dashboard/about-stats/add-about-stat">
                ...
                >
                  ...
                  <td className="px-5 py-3">
                    <div className="flex justify-center gap-2">
                      <Link
                        href={`/dashboard/about-stats/edit-about-stat/${stat.id}`}
                      >
                        <button
