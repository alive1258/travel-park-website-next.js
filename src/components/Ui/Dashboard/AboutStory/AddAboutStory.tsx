...
import { useCreateAboutStoryMutation } from "@/src/redux/api/aboutStoryApi";

interface AddAboutStoryFormValues {
  eyebrow?: string;
  heading: string;
  position?: number;
  is_active: boolean;
...
const AddAboutStory = () => {
  ...
  const [createAboutStory, { isLoading }] = useCreateAboutStoryMutation();
  ...
  const {
    ...
    reset,
    formState: { errors },
  } = useForm<AddAboutStoryFormValues>({
    defaultValues: {
      eyebrow: "Our Story",
      heading: "",
      position: 1,
  ...
  const onSubmit: SubmitHandler<AddAboutStoryFormValues> = async (values) => {
    ...
    try {
      ...
      await createAboutStory(formData).unwrap();
      toast.success("About Story created successfully!");
      reset();
      setImagePreview(null);
      setParagraphs([]);
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="mt-8 space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <Input
            label="Heading"
            text="heading"
            placeholder="A Fleet Built Around a Simple Idea"
        ...
        <div className="flex justify-end gap-4 mt-6">
          ...
          <GradientButton
            type="submit"
            text={isLoading ? "Saving..." : "Create Story"}
            icon={Plus}
            disabled={isLoading}
          />
        </div>
