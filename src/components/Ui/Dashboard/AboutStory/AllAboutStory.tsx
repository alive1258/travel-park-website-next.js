...
import { AboutStoryItem } from "@/src/types/aboutStoryType";
...
const AllAboutStory: React.FC = () => {
  const [searchValue, setSearchValue] = useState("");
  const [currentPage, setCurrentPage] = useState(1);

  const debouncedSearch = useDebounce(searchValue, 500);

  const { data, isLoading, isFetching, refetch } = useGetAllAboutStoryQuery({
    search: (debouncedSearch as string) || undefined,
    page: currentPage,
    limit: LIMIT,
  });

  const [deleteAboutStory] = useDeleteAboutStoryMutation();

  const stories: AboutStoryItem[] = data?.data || [];
  ...
  const handleDeleteStory = async (story: AboutStoryItem) => {
    try {
      const result = await Swal.fire({
        title: "Are you sure?",
        text: `Delete story "${story.heading}"?`,
        icon: "warning",
      ...
      await deleteAboutStory(story.id).unwrap();

      await Swal.fire({
        icon: "success",
        title: "Deleted!",
        text: `Story "${story.heading}" has been deleted.`,
        timer: 1000,
      ...
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 p-6 border-b border-gray-200">
        <div>
          ...
          <p className="text-sm text-gray-500">
            Manage the &quot;Our Story&quot; section on the About page
          </p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3 w-full lg:w-auto">
          ...
          <Link href="/dashboard/about-story/add-about-story">
                ...
                >
                  ...
                  <td className="px-5 py-3">
                    <div className="flex justify-center gap-2">
                      <Link
                        href={`/dashboard/about-story/edit-about-story/${story.id}`}
                      >
                        <button
