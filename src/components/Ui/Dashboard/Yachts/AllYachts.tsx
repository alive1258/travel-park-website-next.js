...
import { YachtAdminItem } from "@/src/types/yachtAdminType";
...
const AllYachts: React.FC = () => {
  const [searchValue, setSearchValue] = useState("");
  const [currentPage, setCurrentPage] = useState(1);

  const debouncedSearch = useDebounce(searchValue, 500);

  const { data, isLoading, isFetching, refetch } = useGetAllYachtsQuery({
    search: (debouncedSearch as string) || undefined,
    page: currentPage,
    limit: LIMIT,
  });

  const [deleteYacht] = useDeleteYachtMutation();

  const yachts: YachtAdminItem[] = data?.data || [];
  ...
  const handleDeleteYacht = async (yacht: YachtAdminItem) => {
    try {
      const result = await Swal.fire({
        title: "Are you sure?",
        text: `Delete yacht "${yacht.name}"? This also removes its hero image and gallery.`,
        icon: "warning",
      ...
      await deleteYacht(yacht.id).unwrap();

      await Swal.fire({
        icon: "success",
        title: "Deleted!",
        text: `Yacht "${yacht.name}" has been deleted.`,
        timer: 1000,
  ...
  return (
    <div className="rounded-xl border border-gray-200 bg-white shadow-sm overflow-hidden">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 p-6 border-b border-gray-200">
        <div>
          <h1 className="text-2xl font-semibold text-gray-900">Yacht Fleet</h1>
          <p className="text-sm text-gray-500">
            Manage every yacht shown on the Fleet and yacht detail pages
          </p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3 w-full lg:w-auto">
          ...
          <Link href="/dashboard/yachts/add-yacht">
              ...
              yachts.map((yacht, index) => (
                ...
                >
                  ...
                  <td className="px-5 py-3 text-sm font-medium text-gray-800">
                    {yacht.name}
                    <div className="text-xs text-gray-400">{yacht.slug}</div>
                  </td>
                  ...
                  <td className="px-5 py-3 text-center text-sm text-gray-600">
                    {yacht.guests} / {yacht.cabins}
                  </td>

                  <td className="px-5 py-3 text-right text-sm text-gray-800 font-medium">
                    {yacht.currency} {Number(yacht.price_per_night).toLocaleString()}
                  </td>
                  ...
                  <td className="px-5 py-3">
                    <div className="flex justify-center gap-2">
                      <Link href={`/dashboard/yachts/edit-yacht/${yacht.id}`}>
                        <button
                      ...
                      <button
                        onClick={() => handleDeleteYacht(yacht)}
