...
import { AboutItem } from "@/src/types/aboutType";
...
const AllAbout: React.FC = () => {
  ...
  const { data, isLoading, isFetching, refetch } = useGetAllAboutQuery({
  ...
  const abouts: AboutItem[] = data?.data || [];
  ...
  const handleDeleteAbout = async (about: AboutItem) => {
    try {
      const result = await Swal.fire({
        title: "Are you sure?",
        text: `Delete about section "${about.name}"?`,
        icon: "warning",
      ...
      await deleteAbout(about.id).unwrap();

      await Swal.fire({
        icon: "success",
        title: "Deleted!",
        text: `About section "${about.name}" has been deleted.`,
        timer: 1000,
