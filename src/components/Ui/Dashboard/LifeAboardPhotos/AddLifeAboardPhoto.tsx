...
import { useCreateLifeAboardPhotoMutation } from "@/src/redux/api/lifeAboardPhotoApi";

interface AddLifeAboardPhotoFormValues {
  position?: number;
  is_active: boolean;
  image?: FileList;
}
...
const AddLifeAboardPhoto = () => {
  const router = useRouter();
  const [imagePreview, setImagePreview] = useState<string | null>(null);

  const [createLifeAboardPhoto, { isLoading }] =
  ...
  const {
    ...
    reset,
    formState: { errors },
  } = useForm<AddLifeAboardPhotoFormValues>({
    defaultValues: {
      position: 1,
      is_active: true,
    },
  ...
  const onSubmit: SubmitHandler<AddLifeAboardPhotoFormValues> = async (
    values,
  ) => {
    if (!values.image?.[0]) {
      Swal.fire({
    ...
    try {
      ...
      await createLifeAboardPhoto(formData).unwrap();
    ...
    <div className="rounded-lg border bg-white border-gray-200 overflow-hidden p-6">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="mt-8 space-y-6">
        ...
        <div className="flex justify-end gap-4 mt-6">
          ...
          <GradientButton
            type="submit"
            text={isLoading ? "Saving..." : "Add Photo"}
            icon={Plus}
            disabled={isLoading}
          />
        </div>
...
export default AddLifeAboardPhoto;
