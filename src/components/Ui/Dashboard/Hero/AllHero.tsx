...
import { HeroItem } from "@/src/types/heroType";
...
const AllHero: React.FC = () => {
  ...
  const { data, isLoading, isFetching, refetch } = useGetAllHeroQuery({
  ...
  const heroes: HeroItem[] = data?.data || [];
  ...
  const handleDeleteHero = async (hero: HeroItem) => {
    try {
      const result = await Swal.fire({
        title: "Are you sure?",
        text: `Delete hero section "${hero.title}"?`,
        icon: "warning",
      ...
      await Swal.fire({
        icon: "success",
        title: "Deleted!",
        text: `Hero section "${hero.title}" has been deleted.`,
        timer: 1000,
    ...
    <div className="rounded-xl border border-gray-200 bg-white shadow-sm overflow-hidden">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 p-6 border-b border-gray-200">
        ...
        <div className="flex flex-col sm:flex-row gap-3 w-full lg:w-auto">
          ...
          <Link href="/dashboard/hero/add-hero">
                ...
                >
                  ...
                  <td className="px-5 py-3">
                    <div className="flex justify-center gap-2">
                      <Link href={`/dashboard/hero/edit-hero/${hero.id}`}>
                        <button
