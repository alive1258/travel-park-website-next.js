...
import { useCreateDestinationMutation } from "@/src/redux/api/destinationApi";

interface AddDestinationFormValues {
  name: string;
  description?: string;
  position?: number;
  is_active: boolean;
...
const AddDestination = () => {
  ...
  const {
    ...
    reset,
    formState: { errors },
  } = useForm<AddDestinationFormValues>({
    defaultValues: {
      name: "",
      position: 1,
      is_active: true,
  ...
  const onSubmit: SubmitHandler<AddDestinationFormValues> = async (values) => {
    try {
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="mt-8 space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <Textarea
            ...
            text="description"
            placeholder="Whitewashed villages and crystal Aegean coves."
        ...
        <div className="flex justify-end gap-4 mt-6">
          ...
          <GradientButton
            type="submit"
            text={isLoading ? "Saving..." : "Create Destination"}
            icon={Plus}
            disabled={isLoading}
          />
        </div>
