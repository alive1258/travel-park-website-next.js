...
interface EditClientVideoReviewProps {
  id: string;
}

interface EditClientVideoReviewFormValues {
  name: string;
  designation?: string;
  description: string;
  rating: number;
...
const EditClientVideoReview: React.FC<EditClientVideoReviewProps> = ({
  id,
}) => {
  ...
  const { data: reviewData, isLoading: isFetching } =
    useGetSingleClientVideoReviewQuery(id);
  const [updateClientVideoReview, { isLoading: isUpdating }] =
    useUpdateClientVideoReviewMutation();
  ...
  const {
    ...
    reset,
    formState: { errors },
  } = useForm<EditClientVideoReviewFormValues>();
  ...
  // from `reviewData` stays put instead of being cleared.
  useEffect(() => {
  ...
  const onSubmit: SubmitHandler<EditClientVideoReviewFormValues> = async (
    values,
  ) => {
    try {
      ...
      await updateClientVideoReview({ id, data: formData }).unwrap();
      toast.success("Client video review updated successfully!");
      ...
      Swal.fire({
        ...
        text:
          (Array.isArray(error.data?.message)
            ...
            : error.data?.message) || "Failed to update client video review.",
        icon: "error",
      });
    }
  };
  ...
  return (
    <div className="rounded-lg border bg-white border-gray-200 overflow-hidden p-6">
      <PageHeader
        title="Edit Client Video Review"
        breadcrumbs={[
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="mt-8 space-y-6">
        ...
        <div className="flex justify-end gap-4 mt-6">
          ...
          <GradientButton
            type="submit"
            text={isUpdating ? "Updating..." : "Update Video Review"}
            icon={Save}
            disabled={isUpdating}
          />
        </div>
...
export default EditClientVideoReview;
